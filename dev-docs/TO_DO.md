...
- [ ] **Getting-started notebook refresh (Phase 3 §3 GAP)** — `docs/source/getting_started/getting_started.ipynb` still uses legacy PySkinDose-as-product prose, stale outputs (including a Windows-specific example RDSR path), and pre-GUI workflow. Refresh for GUISkinDose where the notebook describes *this* product; keep PySkinDose references where they denote upstream lineage or historical plot/API names. Align `main()`/settings examples with `AGENTS.md`.
- [ ] **Glossary plot-axis labels (Phase 3 §6 GAP)** — add LON/LAT/VER (and related PT L-R / A-P / S-I aliases) to `dev-docs/glossary.json` so in-app axis labels have a canonical definition entry.
