...
- [ ] Add support for multiple exams ([plan](plans/multiple-exams.md)).
- [ ] **Convert the per-event recursion to a loop (RecursionError on long/multi-exam data)** — `calculate_dose.calculate_irradiation_event_result` processes events by recursing once per event (it calls itself for `event + 1` at the tail). Python's default recursion limit (~1000) caps this: a single long procedure, and especially **multiple RDSRs/exams loaded together** (a patient with several studies in a short period — see "Add support for multiple exams" above), can exceed ~990 events and raise `RecursionError`, failing the whole calculation. Rewrite as an iterative loop over events (carry `hits`/`table_hits`/`field_area`/`k_isq` across iterations as the recursion currently threads them) so event count is bounded only by memory, not the stack. Verified 2026-06-13 that a 712-event export completes; the headroom is thin. Keep the progress-bar updates and the new-geometry reuse semantics identical. [Plan](plans/recursion-to-iteration.md).
  - [ ] **Review plan** — scan `plans/recursion-to-iteration.md` for errors, gaps, or improvements before implementation. Key concerns: `output_template` shared-reference bug (`[[]] * N`), phantom `position()` in-place mutation, synthetic data generation for stress test, and `event != 0` validation behavioral change.
