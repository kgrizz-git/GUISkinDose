...
- [ ] **GUI network-exposure hardening** — non-loopback refused outright (`_resolve_bind_host()` raises; `--host`/`--allow-network` flags removed): threat model + recorded refusal decision: [  assessment](assessments/GUI_NETWORK_EXPOSURE_ASSESSMENT.md). Follow-up (separate PR, not this branch), ranked by value-per-effort: Origin validation on websocket/state-changing routes + per-launch token first (prerequisite: NiceGUI hardcodes `cors_allowed_origins='*'` on its socket.io server, so this needs a socketio allowed-origins override and/or ASGI Host/Origin middleware before `ui.run()` — not a plain app-level check), strict Host validation second, loopback-scope help note (required registry entry; must cover the malicious-webpage angle) third, randomized loopback port as modest defense-in-depth; per-client state + real auth on a demonstrated clinical-LAN need **or shared/multi-user workstation requirement** (deferred, not scheduled — and then with TLS, authorization, session lifecycle, isolation, and audit, not auth alone).
