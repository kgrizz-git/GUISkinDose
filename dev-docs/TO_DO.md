...
- [ ] **GUI network-exposure hardening** — non-loopback refused outright (`_resolve_bind_host()` raises; `--host`/`--allow-network` flags removed): threat model + recorded refusal decision: [  assessment](assessments/GUI_NETWORK_EXPOSURE_ASSESSMENT.md). Follow-up (separate PR, not this branch): loopback-scope help note, Host/Origin validation, randomized loopback port (residual drive-by-web risk); per-client state + real auth only on a demonstrated clinical-LAN need (deferred, not scheduled).
