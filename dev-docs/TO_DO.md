...
- [ ] **HVL-lookup crash on invalid / out-of-grid events** — **crash fixed** (`geom_calc.fetch_and_append_hvl` now snaps out-of-grid events to the nearest tabulated point instead of `.iloc[0]`-ing an empty lookup; regression test added). The legacy Radimetrics CSV calc completes (2/712 events snapped; count logged). **Remaining** per [hvl-invalid-event-crash.md](plans/hvl-invalid-event-crash.md): surface the snapped/failed count in the GUI (currently console-only), and drop/zero invalid sub-floor-kVp events on tabular loads (`remove_invalid_rows` checks `kVp == 0` exactly and isn't applied on the tabular path). Subsumes the item below.
