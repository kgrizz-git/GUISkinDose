...
- [ ] **Unify the split dependency declarations + license-notices baseline** — deps are declared in two places that disagree: `pyproject.toml` extras (`gui`, `gui-native`, `dev`) vs. the legacy `requirements.txt` / `requirements-dev.txt` / `docs/requirements.txt`. The docs + notebook toolchain (Sphinx, `sphinx-rtd-theme`, `myst-parser`, `nbsphinx`, JupyterLab, `ipywidgets`, `ipykernel`, `pandoc`) lives **only** in the requirements files — it builds the ReadTheDocs site (`.readthedocs.yml` → `docs/source/conf.py`, `docs/Makefile`, `build_documentation.bat`) and renders `docs/source/getting_started/getting_started.ipynb`, but is absent from `pyproject.toml`. Because `scripts/check_licenses.py` generates `dev-docs/THIRD_PARTY_NOTICES.md` from `pip install -e ".[dev,gui]"` (pyproject extras only), any venv that also installed the docs/notebook requirements drifts the notices (extra Sphinx/Jupyter rows, incidental transitive re-resolutions e.g. `aiohttp` 3.14.1→3.14.0), failing the `license-notices` pre-commit hook on unrelated changes. Fix: move the docs/notebook deps into `pyproject.toml` extras (`[docs]`, `[notebooks]`), make the `requirements*.txt` files thin shims over the extras (or drop them), and point `check_licenses.py` at the full declared set so the notices inventory is reproducible regardless of who installed what. Discovered 2026-06-19 while archiving the below-floor-kVp plan.
...
- [x] **User options for below-floor / unresolvable kVp events** — **done** ([hvl-interpolation-and-below-floor-kvp.md](plans/archive/hvl-interpolation-and-below-floor-kvp.md), Phase 2, 2026-06-19): events with **kVp < 25 kV** now honor a user-selectable policy `below_floor_kvp_policy ∈ {snap (default), skip, manual, exam_average}` (+ `below_floor_kvp_manual`), applied per exam in `geom_calc.apply_below_floor_kvp_policy` before the HVL lookup. `snap` keeps the status quo (clamp + flag); `skip` drops the events; `manual` substitutes a fixed kVp; `exam_average` substitutes that exam's mean in-floor kVp (falls back to `snap` + warns if all-below). Settings control under Physics + a pre-calc prompt (`app._below_floor_prompt`) that fires only when sub-floor events are detected; affected-count always reported via `state.calc_warnings`. Remaining: manual GUI smoke check (load a sub-floor export, exercise each policy).
