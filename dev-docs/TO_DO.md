...
- [ ] **Getting-started notebook execution failure in docs builds** — nbsphinx
  ...
  Root cause: plot modes return no output by design but the html branch raised
  unconditionally (regression from 2026-06-07; the dose cells are innocent —
  same calculation returns PSD ~10.4 headlessly). Active fix plan:
  [NOTEBOOK_PLOT_HTML_FIX_PLAN.md](plans/NOTEBOOK_PLOT_HTML_FIX_PLAN.md).
  Residual: confirm no second latent failure behind it in RTD logs (e.g. the
  `tqdm_notebook.disp` incompatibility seen in CI build logs).
