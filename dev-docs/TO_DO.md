...
- [ ] **HVL-lookup crash on invalid / out-of-grid events** — **crash fixed** (`geom_calc.fetch_and_append_hvl` snaps out-of-grid events to the nearest grid point; regression test added) and the snapped-event **count is now surfaced in the GUI** (calculate-tab status line + toast via `state.calc_warnings`). **Remaining** per [hvl-invalid-event-crash.md](plans/hvl-invalid-event-crash.md): drop/zero invalid sub-floor-kVp events on tabular loads (`remove_invalid_rows` checks `kVp == 0` exactly and isn't applied on the tabular path) — has product decisions (threshold, drop-vs-zero, default). Subsumes the item below.
