...
- [x] **Surface event-miss + event-discard notifications in the GUI** — **done** (2026-06-24): beam-miss warnings at WARNING level; see [archived plan](plans/archive/NO_PATIENT_INTERSECTION_WARNING_PLAN.md). After each `Calculate` run, the GUI must notify the user (a) **how many irradiation events had no intersection with the patient phantom** (i.e. `Beam.check_hit` returned an empty `hits` list for that event) and (b) **how many events were discarded and why** (covers, at minimum: `below_floor_kvp_policy="skip"` drops, `fetch_and_append_hvl` out-of-grid events flagged `clamped` or `interpolated`, and any other event the orchestrator skips). Both counts ride the existing `state.calc_warnings` toast flow in `gui/tabs/calculate.py:233-238` (one toast per category) plus a one-line summary on the Calculate-tab status label, e.g. `Done — PSD 123.4 mGy · 3 beam-miss event(s) (events 7, 12, 41) · 5 discarded event(s) (4 below-floor skipped, 1 out-of-grid clamped)`. Empty categories are omitted. Per-event detail stays in the per-event warnings list / Results tab accordion. [Plan](plans/NO_PATIENT_INTERSECTION_WARNING_PLAN.md)
- [ ] **More patient sizes / phantom body-habitus scaling** — the bundled human meshes (`phantom_data/*.stl`: adult/junior/senior male/female, hudfrid) cover a narrow size range with no obese/large-habitus option. Two complementary approaches: (a) **generate or source additional meshes** for under-represented body types (e.g. obese), keeping the existing full-res + `_reduced_1000t` pairing and wiring them into `get_human_mesh_names()`; and/or (b) **directional scaling of the current phantoms** — let the user stretch a mesh along lateral / AP / longitudinal axes (e.g. anthropometric sliders or a BMI/girth control) by scaling the STL vertices in `phantom_class.py` before dose mapping. Body size changes skin-to-source distance and beam intersection, so this materially affects PSD; surface it as a control in the Settings → Phantom section. Validate scaled geometry in the Geometry tab. [Plan](plans/PATIENT_SIZE_SCALING_PLAN.md)
...
- [ ] **Show slider values for patient and table offsets in the Geometry tab** — patient offset sliders already show their current value (e.g. "0.0 cm") next to each slider (`gui/tabs/geometry.py:175`); table-origin override sliders (X/Y/Z) do not. Mirror the patient-slider pattern: add a small mono-text value label under each table-origin slider showing `{val:.1f} cm`, updated in `_on_table_slider` and `_sync_table_sliders_from_meta`.
- [ ] **Clarify vendor-specific X/Z on the Geometry plot** — on the Geometry preview the X and Z axes can mean different things for different manufacturers (e.g. GE vs. Philips vs. Siemens coordinate conventions — see `VENDOR_COORDINATE_SYSTEMS.md`), which makes the plot orientation confusing when comparing data from different vendors. Pick one: (a) **convert for display** so X/Z always mean the same thing on the plot (e.g. always patient-frame lateral/vertical, regardless of source vendor), or (b) **add an in-app note** explaining why the axes differ (banner or caption near the plot, plus a link to `VENDOR_COORDINATE_SYSTEMS.md`), with a clear axis legend reflecting the active vendor. Also consider the implications for the patient/table offset sliders and the table-origin X/Y/Z labels (currently axis-named, not semantic).
...
- [ ] **Native window: default to full-screen on launch, with an in-GUI opt-out** — `--native` (`gui/app.py:147`, `main.py:332`) currently opens at 75% of screen size. Make it start full-screen by default, and add a GUI option (location TBD: Settings tab expansion, Visual Settings, or header) with a checkbox like "Start native window full-screen" to disable full-screen on next launch. Persist the preference to a small config file (e.g. `~/.mypyskindose/gui.json`) so the choice survives restarts.
