...
- [ ] **(NEXT) HVL-lookup crash on invalid / out-of-grid events** — calculating on the legacy Radimetrics CSV aborts with `IndexError` in `geom_calc.fetch_and_append_hvl` (`.iloc[0]` on an empty exact-match lookup). Measured: 710/712 events fine, 2 fail because kVp ≈ 0.003 rounds to 0 (below the 25 kV table floor); `remove_invalid_rows` checks `kVp == 0` exactly (misses 0.003) and is not applied on the tabular path. Plan: [hvl-invalid-event-crash.md](plans/hvl-invalid-event-crash.md) — guard the lookup (nearest/interpolate, never bare `.iloc[0]`), drop/zero invalid-kVp events on tabular loads too, fail soft per-event, warn the user, add a regression test. Subsumes the item below.
- [ ] **User options for below-floor / unresolvable kVp events** — once the calc fails soft (see plan above), offer the user a choice for events whose kVp is below the HVL table floor (or otherwise can't be resolved) rather than silently dropping them: (a) ignore/skip the event, (b) enter a kVp value manually, or (c) substitute the exam's average kVp. GUI surface (e.g. a prompt or a per-event control); builds on [hvl-invalid-event-crash.md](plans/hvl-invalid-event-crash.md). Whichever path is taken, always report **how many events were affected**.
...
- [ ] **Confirm the uploader-wedge fix and resolve** — dragging a file *out of the native file-open dialog* onto the drop zone wedged quasar in a phantom upload (`0.0B/0.00%`, `+` disabled); `uploader.reset()` did **not** recover it, so the "Reset upload" button / card X now **rebuild** the uploader element (commit `37019e1`). Re-test the exact path in a real browser: trigger the wedge, click **Reset upload**, confirm `+` works again. If rebuild recovers it, close this out; if not, escalate (investigate quasar `added`/`abort` events to *prevent* the 0-byte phantom). Note: this is manual-smoke only — no drag-drop simulation in the test suite.
