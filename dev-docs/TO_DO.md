...
- [ ] **GUI network-exposure hardening** — non-loopback refused outright (`_resolve_bind_host()` raises; `--host`/`--allow-network` flags removed): threat model + recorded refusal decision: [  assessment](assessments/GUI_NETWORK_EXPOSURE_ASSESSMENT.md). Follow-up (separate PR, not this branch), ranked by value-per-effort: Origin validation on websocket/state-changing routes + per-launch token first, strict Host validation second, loopback-scope help note (must cover the malicious-webpage angle) third, randomized loopback port as modest defense-in-depth; per-client state + real auth only on a demonstrated clinical-LAN need (deferred, not scheduled — and then with TLS, authorization, session lifecycle, isolation, and audit, not auth alone).
