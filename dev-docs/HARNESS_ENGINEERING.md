...
| CodeRabbit | Auto-review off (`.coderabbit.yaml`); path filters exclude sensitive surfaces; `request-coderabbit` job posts `@coderabbitai review` after `privacy-gates` on non-draft PRs (deduped per head SHA). The CI workflow also listens for `ready_for_review`, so moving a draft PR to ready triggers its privacy-gated request without an extra push. Manual CodeRabbit requests bypass the CI ordering, so this is not a trusted privacy boundary. |
