...
| Stale-brand allowlist (GUISkinDose rename) | `scripts/check_stale_brand.py` (PR 0 still allows live `mypyskindose`; tighten `LIVE_PACKAGE_NAME` in PR 1) |
...
| **check-stale-brand** | `python scripts/check_stale_brand.py` (leftover `mypyskindose` / `MyPySkinDose` / `MYPYSKINDOSE_` outside the allowlist; live package name still permitted in PR 0) |
