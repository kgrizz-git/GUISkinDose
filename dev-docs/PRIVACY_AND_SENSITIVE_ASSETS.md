...
Runtime logging follows a what-we-log model, not a blanket redaction filter: console and any future file sink share the same logger tree, formatters, and call sites, and privacy is enforced by call-site discipline plus handler levels. GUI/CLI boundaries use `safe_error_event()` / `safe_user_error()` (operation code + exception class only, value-free tracebacks at DEBUG with package-relative paths); uncaught CLI exceptions go through `install_value_safe_excepthook()`; GUI load failures follow the same pattern (`gui/exam_loaders.py`). Routine INFO/WARNING lines avoid identifiers and source paths — uploads and RDSR reads log at most the file suffix (plus byte count for uploads) at DEBUG, never filenames; prefer `safe_warning()` for coded metrics. Opt-in `dprint` DEBUG categories write to stderr and are developer toggles, not clinical-safe output. A future file sink defaults to INFO (DEBUG only with an explicit category), rotates (~1 MiB × 4, session purge) with POSIX `0o600`; a settings/CLI toggle to disable file logging on shared machines is still open (see the native file-logging item in `dev-docs/TO_DO.md`).
