...
- [ ] Determine and document whether `device_info.csv` should be retained as
  ...
  **Decision (2026-09-18, implemented in Phase A):** substitute the serial/lab
  identifiers with obviously-synthetic tokens (token scheme in the Phase A
  execution plan — deliberately not a retained mapping), preserve
  ...
  hash-pinned binaries would invalidate clearance and force updates across
  golden/regression suites for no propagation gain. The misleading "Synthetic labels only" comment in
  `test_rdsr_parser_station.py:25` is corrected to state the values are retained
  from upstream.
