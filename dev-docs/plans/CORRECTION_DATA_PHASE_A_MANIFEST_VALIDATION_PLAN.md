...
1. **`device_info.csv` substitution spec** — `DeviceObserverSerialNumber` →
   `SYN-SN-01…04`; `Lab` → `SYN-LAB-A…D` (distinct per row/lab);
   `PadThickness_mm` (74/74/101/68), `DateMeasured`, `PadMaterial` preserved;
   `Comment` preserved with lab refs rewritten ("estimated from lab 105" →
   "estimated from SYN-LAB-A"). Schema, row order, column set unchanged.
...
3. **Comment correction** — `test_rdsr_parser_station.py:26` "Synthetic labels
...
- New validation module imports stdlib + pandas (+ `guiskindose.privacy` for
  error paths) only, so `test_architecture_layers.py` stays green; nothing in
  `settings/` may import it.
...
- [ ] **Step 3: Explicit-DB validators + drift matrix** — implement §2 adapter
  ...
  table/column and never create files. No runtime wiring (Phase B).
- [ ] **Step 4: device_info substitution + comment fix** — apply the locked
  ...
  `pytest -q` green with zero golden drift (no numeric change possible — no
  consumer reads the swapped columns — but prove it).
- [ ] **Step 5: Docs + verification ladder** — CHANGELOG brief entry +
  ...
  → `check_changelog` → `audit_dependencies` (lock untouched; expect green).
...
- `device_info.csv` carries no real identifiers; `PadThickness_mm` intact.
- All pre-existing golden/dose tests pass unmodified (zero behavior change).
- Diff: manifest + validation module + tests + CSV swap + comment + changelog/
  maintenance-log/index docs. No runtime, settings, export, or lock changes.
