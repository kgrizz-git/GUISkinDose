...
| **Where applied** | At **normalization** time — [`rdsr_normalizer.py`](../../../src/guiskindose/rdsr_normalizer.py) builds `Tx/Ty/Tz` from `norm.trans_offset` + `norm.trans_dir`. | At **dose-calc** time — `geom_calc.py` `patient.translate(dr=patient_offset)`. |
...
The conventions live in [`normalization_settings.json`](../../../src/guiskindose/normalization_settings.json), matched per file by `(manufacturer, model)` in [`normalization_settings.py`](../../../src/guiskindose/settings/normalization_settings.py) (`update_used_settings`). **The convention does vary by provider** — the shipped table shows it concretely:
...
- `rot_dir.Ap3`, `rot_dir.At1`–`At3` → **no lever, and none needed**: those columns are hard-zeroed in [`rdsr_normalizer.py`](../../../src/guiskindose/rdsr_normalizer.py) (`rot_dir.* × [0]`), so their sign is a no-op.
...
- `rot_dir.Ap3`, `At1`–`At3` → always zero in [`rdsr_normalizer.py`](../../../src/guiskindose/rdsr_normalizer.py), so overriding their sign is a no-op — nothing meaningful to expose.
