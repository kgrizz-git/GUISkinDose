...
Make Geometry tab X/Y/Z controls mean the same plotted axes for every vendor and import path. A user changing the `X/LON` table-origin control must move the plotted X axis whether the exam is Siemens-like, Philips-like, GE normalized with `swap_lateral_longitudinal`, or a tabular export with manual post-normalization coordinate corrections.
...
The implementation may store manual table-origin overrides in the GUI transform source frame, but the Geometry and Per-exam UI must never expose raw/source-frame behavior. UI reads and writes must be mapped to the final plotted frame after manual `swap_lat_lon` and axis flips.

This plan does **not** relabel the plotted axes into the physical/anatomical frame documented in `dev-docs/VENDOR_COORDINATE_SYSTEMS.md` and tracked by `dev-docs/plans/COORDINATE_CONVENTIONS_CLEANUP_PLAN.md`. The code path has been traced through `rdsr_normalizer._normalize_table_parameters()`, `Beam.__init__()`, `geom_calc.position_patient_phantom_on_table()`, `Phantom.position()`, and `calculate_dose()`: the current plot and hover labels still use historical PySkinDose aliases where coordinate index 0 is shown as `LON` and coordinate index 2 is shown as `LAT`. Renaming the visible axes to physical `X - LAT` / `Z - LON` belongs to the coordinate-conventions cleanup work, not this targeted Geometry-control fix.
...
`rdsr_normalizer._normalize_table_parameters()` currently assigns normalized table-position columns from parsed RDSR/tabular fields after manufacturer-specific offset, sign, and optional lateral/longitudinal swap handling:

| Derived col | Plot axis | Current display alias | Source DICOM tag |
|-------------|-----------|-----------------------|------------------|
| `Tx` | X | `LON` | usually `TableLongitudinalPosition_mm`; for manufacturer rules with `swap_lateral_longitudinal`, e.g. GE, sourced from `TableLateralPosition_mm` |
| `Ty` | Y | `VER` | `TableHeightPosition_mm` |
| `Tz` | Z | `LAT` | usually `TableLateralPosition_mm`; for manufacturer rules with `swap_lateral_longitudinal`, e.g. GE, sourced from `TableLongitudinalPosition_mm` |
...
Therefore, the current plots and hovers are internally consistent in the historical PySkinDose alias frame. The confusion comes from the mismatch between those aliases, physical patient directions, DICOM table-coordinate attribute names, and the lack of in-app explanation.
...
- The **tabular import** `swap_lat_lon` toggle exists (`AppState.swap_lat_lon`, applied in `gui.exam_transforms._apply_transform_flags()`), but it is now a manual expert post-normalization override. GE should not require this toggle by default because the GE wildcard in `normalization_settings.json` uses `swap_lateral_longitudinal: true`.
- **Table-origin overrides currently live in the detected GUI transform source frame, but the UI does not map them to the final plotted frame.** `_apply_transform_flags()` applies `table_origin_override` to `Tx/Ty/Tz` before manual `swap_lat_lon` swaps `Tx` and `Tz`, while `effective_table_origin()` and `stage_table_origin_axis()` read/write bare `x`, `y`, `z` keys. For any manually swapped tabular/site-specific import, changing the Geometry tab `X` table-origin override can therefore affect the final plotted Z coordinate. That is the core user-facing bug this plan must prevent. For GE, this must also prevent users from applying a second GUI swap on top of the normalizer-level correction.
...
### Option A — Relabel axes into the physical/anatomical frame
...
For table-origin overrides, use **GUI-transform-source-frame storage with final-frame UI mapping**:

- Keep `state.loaded_exam_meta[*]["table_origin_detected"]` and `["table_origin_override"]` in the GUI transform source frame so a physical manual origin follows a later manual `swap_lat_lon` toggle instead of silently staying attached to the old displayed axis.
- Add helper functions that transform GUI-transform-source-frame origin dicts into the final plotted frame for display, slider limits, and audit text. For origin values, manual `swap_lat_lon` transposes X/Z; sign flips must be treated carefully because current `flip_tx`/`flip_ty`/`flip_tz` behavior pivots about the detected origin and reverses table motion without moving the origin coordinate itself.
- Make `stage_table_origin_axis(meta, axis, value)` accept a final-frame axis/value from the UI and inverse-map it into the stored GUI-transform-source-frame override before `_apply_transform_flags()` runs.
- Keep `_apply_transform_flags()` applying the stored GUI-transform-source-frame override before manual `swap_lat_lon`, because the UI mapping layer is what makes the controls vendor-invariant.
...
1. **Phase 1 starts with the table-origin frame mapping fix**: labels/help may ship in the same phase, but the phase is not complete until Geometry table-origin controls move the same plotted axis across Siemens-like, Philips-like, GE-normalized, and manually corrected tabular exams.
...
1. **Table-origin override frame mapping fix** — in `src/mypyskindose/gui/exam_transforms.py` and `src/mypyskindose/gui/offset_handlers.py`, keep origin storage in the GUI transform source frame but make all UI controls final-frame:
   - keep `state.loaded_exam_meta[*]["table_origin_detected"]` and `["table_origin_override"]` stored as GUI-transform-source-frame `{"x", "y", "z"}` keys
   - add GUI-source↔final origin mapping helpers that transpose X/Z for manual `swap_lat_lon`; document that `flip_tx`/`flip_ty`/`flip_tz` reverse table motion about the detected origin and therefore do not simply sign-flip origin coordinates
   ...
   - make `stage_table_origin_axis(meta, axis, value)` accept a final plotted-frame `axis`/`value` and inverse-map it into the stored GUI-transform-source-frame override
   - update `_table_origin_override_note(meta)` so audit text states that stored values are in the GUI transform source frame and, when different, reports final plotted-frame values
   - keep `_apply_transform_flags()` applying the stored GUI-transform-source-frame origin override before manual `swap_lat_lon`
...
8. **Cross-tab table-origin synchronization** — preserve the existing shared-state behavior between Geometry and Settings -> Per-exam corrections while changing the mapping helpers:
   ...
   - coordinate-correction toggles such as manual `swap_lat_lon` must refresh both tabs so displayed final-frame origin values transpose immediately
...
| DICOM RDSR GE examples | no bundled GE DICOM fixture currently exists; GE convention confirmed from tabular export inspection | when a real GE RDSR or matched de-identified fixture is obtained, use it to pin exact regression values for the existing normalizer-level `swap_lateral_longitudinal` path, not to decide whether the GE convention is an open question |
| `generic_rdsr_like` | synthetic RDSR-shaped fixture exists | confirm round-trip against source RDSR parser output |
| `radimetrics` | synthetic Siemens fixture exists; gitignored local GE Radimetrics-style exports exist in `test_data_gitignored/` | characterize the local GE exports as private validation inputs for schema detection, GE warning behavior, and normalizer-level GE correction; assume Radimetrics passes RDSR coordinates through unless a fixture proves otherwise; matched source RDSR/export is deferred fixture confirmation |
...
2. **Document the transform stage** for each adapter:
   ...
   - GUI post-normalization toggles (`swap_lat_lon`, `flip_ap1`, `flip_ap2`) must be described as manual expert coordinate-correction overrides, not adapter validation and not the default GE path
...
3. **Philips and GE risk check** — explicitly document the double-correction risk:
   ...
   - GE: `swap_lateral_longitudinal` is applied during normalization; a second GUI `Tx`/`Tz` swap should be treated as a manual expert override only and can double-correct otherwise valid GE inputs
   - manually swapped site-specific tabular exports: `Tx`/`Tz` swap can be missed or applied twice
...
2. **Unknown-manufacturer warning** — show a Geometry tab banner when normalization falls back to `Default` or when a tabular export lacks coordinate-convention confidence. The banner should point users to manual offset review in Settings / Per-exam corrections. `_C1_BANNER` is a UI pattern reference only; do not reuse its text.
   ...
   - For GE-normalized inputs, explicitly mention that GE lateral/longitudinal handling is already applied during normalization and the GUI `Tx`/`Tz` swap should remain off unless validating a site-specific export proven to need an extra correction.
   - For manually swapped tabular exports, explicitly mention the `Tx`/`Tz` swap risk and whether the GUI correction toggle is active.
...
| Manual-swapped table-origin behavior | `gui.exam_transforms` / `gui.offset_handlers` | With manual `swap_lat_lon=True`, call `stage_table_origin_axis(meta, "x", final_detected_x + 10)` and assert final plotted `Tx` changes by 10 while final `Tz` does not receive that X delta. Repeat the inverse for Z. |
| Toggle-after-override behavior | `gui.exam_transforms` / `gui.offset_handlers` | Set a GUI-transform-source-frame table-origin override with `swap_lat_lon=False`, then toggle `swap_lat_lon=True`; assert `effective_table_origin(meta)` transposes the displayed X/Z values and the stored GUI-transform-source-frame override still composes correctly through `_apply_transform_flags()`. |
...
| Local GE tabular characterization | `test_data_gitignored/` + `radimetrics` adapter | When available locally, run the gitignored GE Radimetrics-style exports through schema detection/normalization and record non-sensitive aggregate coordinate ranges plus warnings; assert GE normalization does not require a second GUI `swap_lat_lon`; do not commit private exports. |
...
- Can any local gitignored GE Radimetrics-style export be paired with its original de-identified DICOM RDSR? This is deferred fixture confirmation for exact raw/regression values, not an open live question about the GE convention or a reason to suspect Radimetrics changes the RDSR coordinate frame.
- Should `--validate-coordinates` be its own plan after Phase 2 documents the validation matrix? Current recommendation: yes.

## 10. 2026-06-27 Coordinate Update Disposition

Recent coordinate-reference and normalization updates were reviewed against this plan. Applied findings:

- GE lateral/longitudinal handling is now normalization-level behavior via `normalization_settings.json` `swap_lateral_longitudinal: true` and `rdsr_normalizer._normalize_table_parameters()`, not a default GUI `swap_lat_lon` workflow
- GE table-travel convention is treated as established from tabular export inspection for head-first supine positioning: positive lateral = patient left, positive longitudinal = patient superior/cranial, and positive height = down; this is a consistent right-handed convention when left is `+x`, down is `+y`, and superior/cranial is `+z`
- matched GE DICOM RDSR plus tabular export remains deferred fixture confirmation for exact raw/regression values, not an open live question about the GE convention
- Radimetrics is treated as an RDSR coordinate pass-through that changes field names/units unless future fixture evidence proves otherwise; do not describe Radimetrics as suspected of changing coordinates
- GUI `swap_lat_lon` remains a manual expert post-normalization override for site-specific tabular/export issues and should not be auto-enabled for GE-normalized inputs
- the table-origin mapping fix remains necessary because any manual `swap_lat_lon` still composes after `table_origin_override`, so Geometry and Per-exam origin controls must map between GUI transform source frame and final plotted frame

## 11. 18:04 Assessment Disposition
...
## 12. New Assessment Disposition

The 2026-06-25 `tmp/assessment_vendor_xz_plan_20260625.md` assessment was reviewed against the code. Applied findings:

- accepted the table-origin state-frame concern: overrides should remain in the GUI transform source frame internally, with tested final-frame UI mapping for Geometry and Per-exam controls
...
- strengthened unknown/fallback, Philips-like, GE-normalized, and manually swapped warning-banner requirements
...
- Do not expose GUI-transform-source-frame table-origin controls directly. The accepted design is GUI-transform-source-frame storage plus final-frame UI mapping, because the primary requirement is that user-facing X/Y/Z controls move the same plotted axes across vendors.

## 13. Previous Assessment Disposition
