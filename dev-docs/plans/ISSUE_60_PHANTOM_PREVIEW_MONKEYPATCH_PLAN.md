...
Status: Active — plan written, awaiting DeepSeek review BEFORE implementation.
Created: 2026-09-17
...
1. GUI tests use NiceGUI's `user` fixture (`tests/conftest.py:22-27`), whose
   teardown runs `nicegui_reset_globals()` (`nicegui/testing/general.py:83-86`,
...
artifact; the pytest-9.1.1 hypothesis is dropped (9.0.3→9.1.1 `resolve()` diff
is `__import__`→`importlib.import_module`, behaviorally identical).
...
- `test_gui_offset_reset.py:225,252,399`
  (`patch("guiskindose.gui.…")`) — closer analogue (same `pkgutil.resolve_name`
  pattern) but green today by fresh-import luck. Leave green tests alone;
  watch note only.
...
- [ ] **Step 2: Apply the object-form fix**
  ...
  existing import block (`:17-23`); replace the string target at `:63-66`
  with `monkeypatch.setattr(mod, "_PHANTOM_DATA_DIR", tmp_path)`.
...
- [ ] **Step 4: Maintenance log + docs**
  ...
  object-form fix). `dev-docs/index.md` execution-plans row for this plan.
  No `CHANGELOG.md` (PR98 exemption: tests-only + maintenance-log touch).
- [ ] **Step 5: Push branch, open PR, dispatch canary**
  `gh workflow run ci-latest.yml` (dispatch enabled, `ci-latest.yml:20`).
  Green auto-closes #60 (`ci-latest.yml:173-213`). If red with the SAME
  signature → fix incomplete, reopen Step 2. If red with a NEW failure →
  separate triage, not scope expansion here.
- [ ] **Step 6: Closeout comment + archive**
  ...
  Archive this plan to `dev-docs/plans/archive/` and update `index.md` in the
  same PR.
