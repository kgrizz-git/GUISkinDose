...
| [PSD_CALCULATION_ALGORITHM.md](PSD_CALCULATION_ALGORITHM.md) | **Source of truth** — PSD dose algorithm in execution order (events → transforms/offsets → corrections → projection → accumulation → PSD). Machine-checked by `tests/unittests/test_psd_algorithm_doc.py`. |
...
| [assessments/DOCUMENTATION_TOOLING_EVALUATION_2026-09-06.md](assessments/DOCUMENTATION_TOOLING_EVALUATION_2026-09-06.md) | Dual-agent spike: Mintlify free-tier/OSS terms, end-user docs tooling survey, ranked shortlist; single-sourced claims verified against primary sources 2026-09-06. Superseded by the decision record below. |
...
| [plans/ROTATIONAL_COVERAGE_ENVELOPE_PLAN.md](plans/ROTATIONAL_COVERAGE_ENVELOPE_PLAN.md) | **Design accepted, not implemented** (2026-09-22) — pointwise full-kerma coverage envelope as the rotational default (detection classifier, candidate domains, controls, disclosures, acceptance); supersedes the assumed-arc sketch, retains scenarios beside it. |
