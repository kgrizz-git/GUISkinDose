...
| [plans/KERMA_METER_CORRECTION_FACTORS_PLAN.md](plans/KERMA_METER_CORRECTION_FACTORS_PLAN.md) | **Active — not started** — apply user-supplied kerma-meter correction factor (`CF = real_measured / unit_reported`) to each event's `K_IRP`, keyed by individual unit (`StationName`/`DeviceSerialNumber`) and tube (`acquisition_plane` A/B/single); fail-soft to CF=1.0; lookup file (CSV/TSV/XLSX/JSON) or pre-calc GUI prompt. |
