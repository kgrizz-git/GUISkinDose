...
`src/guiskindose/table_data/correction_data_manifest.json`. Plan history:
`plans/archive/` (packaging/provenance master, Phase A, Phase B).
...
flowchart TD
    ...
    HVL --> ACC["add_correction_and_event_dose_to_output"]
    KMED --> ACC
    KTAB --> ACC
...
| `device_info` | Bootstrap only | None (provenance only) | — | — | Manifest consistency + substitution tests |
...
| `backscatter`, `h` columns | — | None (`k_bs` uses hard-coded polynomials + `CubicSpline` in `corrections.py:108-121`) | — | — | Manifest `runtime_read: false` flags |

## Key behaviors

- **No CWD writes.** Default runs never create files; an ignored root/CWD
  `corrections.db` warns once per process (value-free) and is never loaded.
- **Explicit databases** validate read-only under the `legacy` policy
