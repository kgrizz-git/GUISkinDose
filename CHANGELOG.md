...
- **Tabular input Phase 5 (partial)** (2026-06-10): GUI upload tab now accepts `.csv`, `.tsv`, `.xlsx`, `.xlsm` in addition to `.dcm`; routes to `load_tabular()` helper; schema selector dropdown (Auto-detect / Normalized / Raw RDSR-like); import preview panel showing schema, encoding, delimiter, header row index, column mapping table, warnings, and first 5 normalized events; lat/lon swap toggle (swaps `Tx`↔`Tz` for GE/DoseTrack Philips exports); Calculate tab blocked on import errors; Data Table tab shows source filename and schema.
