...
- **Privacy admission:** Do not commit PHI/PII, internal PACS endpoints, private-network addresses (IPv4 or IPv6), or diagnostic artifacts. Run the sensitive-content gate; the commit message is checked separately at `commit-msg`. Protected ignore rules/never-track roots and conditional scanner receipts are enforced by `scripts/privacy_admission.py`; run `python scripts/privacy_admission.py run --mode staged` when the route requires it. Images, DICOM, PDFs, and supported archive/document containers require hash-pinned human clearance. See `dev-docs/PRIVACY_AND_SENSITIVE_ASSETS.md` before adding fixtures or handling findings.
