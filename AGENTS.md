...
- **GUI help files:** The canonical source for in-app help markdown is `docs/source/gui_help/`. These files are mirrored to `src/guiskindose/gui/help/` by `scripts/sync_gui_help.py` (enforced by pre-commit + CI). Edit the source under `docs/`, never the mirrored copies under `src/`. Same pattern for UI copy: `dev-docs/ui_copy.json` is canonical, mirrored to `src/guiskindose/gui/ui_copy.json` by `scripts/sync_ui_copy.py` (also enforced) because `dev-docs/` never ships in wheels and `copy_text()` runs on every page render.
...
- **Branch workflow (no direct-to-main):** Never commit or push directly to `main` — all changes, however trivial (including docs-only edits), go on a feature branch and reach `main` via pull request and user merge. The sole exception is an explicit user instruction to push a specific change to `main` for that instance only; such permission never carries over.
