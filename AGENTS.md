...
4. Tabular input Phases 1–4 are **shipped**: `input_adapters/` handles `.csv`, `.tsv`, `.xlsx` via `normalized`, `generic_rdsr_like`, `radimetrics`, and `dosetrack` schemas. DoseTrack adapter: Equipment Name → Manufacturer inference (`MODEL2MANUF`), ffill, integer Plane Code normalization, unit conversions (Air Kerma mGy→Gy, DAP Gy·cm²→Gy·m², Tube Current µA→mA), CFA derivation from DAP formula, Siemens/Philips filter thickness handling, Philips lat/lon swap warning. Validated against synthetic AXIOM-Artis fixture; needs real DoseTrack XLSX for full sign-off. CLI flags `--input-schema`, `--sheet-name`, `--input-preview-only` are wired. The GUI upload tab accepts all tabular formats with import preview, schema selector, lat/lon swap toggle, and provenance in JSON/HTML exports. Phase 5 GUI work is partially shipped. Qaelum and DoseMonitor are documented as Phase 5+ placeholders (need real export fixtures). See `dev-docs/TABULAR_RDSR_INPUT_PLAN.md`, `dev-docs/references/`, and `dev-docs/COORD_TRANSFORM_COMPARISON.md`.
