...
- **Privacy admission:** Do not commit PHI/PII, internal PACS endpoints, private-network addresses, or diagnostic artifacts. Run the sensitive-content gate; the commit message is checked separately at `commit-msg`. Images, DICOM, PDFs, and supported archive/document containers require hash-pinned human clearance. See `dev-docs/PRIVACY_AND_SENSITIVE_ASSETS.md` before adding fixtures or handling findings.
