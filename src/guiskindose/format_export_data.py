...
def format_analysis_result_for_export(
    ...
    ----------
    analysis_result : dict[str, Any]
        The dict resulting from the call to guiskindose.calculate_dose.calculate_dose.calculate_dose
    data_norm : pd.DataFrame
    ...
    """
    guiskindose_output = PySkinDoseOutput(
        patient=patient,
        table=table,
        pad=pad,
    ...
    if settings.output_format == RUN_ARGUMENTS_OUTPUT_DICT:
        return guiskindose_output.to_dict()

    if settings.output_format == RUN_ARGUMENTS_OUTPUT_JSON:
        return guiskindose_output.to_json()
