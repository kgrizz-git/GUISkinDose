...
def classify_rotational_event(event: RotationalEventInput | dict) -> RotationalClassification:
    ...
    meaning = _normalize_meaning(data.acquisition_type_meaning or data.acquisition_type)
    code = _normalize_code(data.acquisition_type_code)
    scheme = _normalize_meaning(data.acquisition_type_coding_scheme)
    code_effective = code if (code and (not scheme or scheme == "dcm")) else ""
    ...
    usable_baseline = ap1 is not None and ap2 is not None
    ...
    if is_stepping:
        ...
        confidence = "coded" if code_effective else ("meaning" if meaning else "none")
    else:
        classification = "unknown"
        confidence = "none"
        if not reasons:
