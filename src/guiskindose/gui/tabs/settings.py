...
# match guiskindose.constants.BELOW_FLOOR_KVP_POLICIES.
...
def _build_phantom_section(ctx: PageContext, on_change: Callable[[], None]) -> None:
    """Phantom Settings expansion: model, mesh, offsets, and habitus scaling.

    Wires ``ctx.refresh_phantom_preview`` to the in-section preview controller so
    every pose-changing control refreshes the Phantom preview via ``on_change``.
    """
        ...
        with ui.column().classes(_SETTINGS_SECTION_CLASSES):
            ...
            with ui.row().classes("w-full gap-6"):
                ui.select(PHANTOM_MODELS, label="Phantom model", value=state.phantom_model).bind_value(
                    state, "phantom_model"
                ).on(_MODEL_VALUE_EVENT, on_change).classes("grow")
            ...
            ).on(_MODEL_VALUE_EVENT, on_change).classes("w-full")
...
def _build_visual_section() -> None:
    """Visual Settings expansion: auto-render toggle + dose map colorscale."""
