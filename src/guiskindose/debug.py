...
* Each debug *category* (GUI/PROCESSING/CALCULATION/RENDERING) maps to a child
  logger ``guiskindose.<category>``. ``dprint`` emits at DEBUG level on that
...
* ``configure_logging()`` sets up the ``guiskindose`` logger tree (console plus
...
# Debug categories — each maps to a child logger ``guiskindose.<category>``.
DEBUG_FLAGS = {
    "GUI": False,
    "PROCESSING": False,
    "CALCULATION": False,
...
def configure_logging(log_file: str | Path | None = None, *, force: bool = False) -> None:
    """Configure the ``guiskindose`` logger tree. Idempotent unless *force*.
...
def dprint(category: str, *args, **kwargs) -> None:
    ...
    Now routed through ``logging.getLogger('guiskindose.<category>').debug()``;
